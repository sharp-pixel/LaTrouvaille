...
Keep the Query understanding toggle available on desktop and mobile and enabled by default. Disabling it must refetch active results, skip the native LLM pipeline, deterministic intent/facet/control extraction, Tier-2 rewrite rules, and persona behavior server-side and in the local fallback. The bypass path executes the literal shopper tokens with OR semantics against lexical OpenSearch, ranks multi-token matches more highly, applies only explicit UI filters, price, and sort, and does not render an inferred-intent strip.
